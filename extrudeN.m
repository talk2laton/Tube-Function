function [X,Y,Z] = extrudeN(CrssSctn,Paramtricfxn,tbound,Nt)
% This function generates the X,Y and Z coordinates to be used by the surf 
% function to generate the intended extruded figure.
% CrssSctn is the cross section to be exruded, X = CrssSctn(:,1), Y = CrssSctn(:,2); 
...
Trans = Paramtricfxn(T); One = ones(Nt+1,1); 
X = One*CrssSctn(:,1)'; Y = One*CrssSctn(:,2)'; Z = zeros(size(X));
